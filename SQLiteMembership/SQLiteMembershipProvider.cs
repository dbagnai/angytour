...
namespace TechInfoSystems.Data.SQLite
{
	...
	public sealed class SQLiteMembershipProvider : MembershipProvider
	{
        ...
        //public override HashAlgorithmType
        //{
		...
		private string EncodePassword(string password, MembershipPasswordFormat passwordFormat, string salt)
		{
			...
			switch (passwordFormat)
			{
				...
				case MembershipPasswordFormat.Hashed:
                    ...
                    //KeyedHashAlgorithm algorithm = KeyedHashAlgorithm.Create(Membership.HashAlgorithmType);
                    //algorithm.Key = src;
                    HashAlgorithm algorithm;// = HashAlgorithm.Create(Membership.HashAlgorithmType);
                    switch (Membership.HashAlgorithmType)
                    {
                        case "HMACSHA256":
                            algorithm = new HMACSHA256(src);
                            break;
                        case "HMACSHA384":
                            algorithm = new HMACSHA384(src);
                            break;
                        case "HMACSHA512":
                            algorithm = new HMACSHA512(src);
                            break;
                        default:
                            algorithm = HashAlgorithm.Create(Membership.HashAlgorithmType);
                            break;
                    }
                    if (algorithm == null)
					{
